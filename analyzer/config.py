...
class Config:
    ...
    # --- S7comm: профиль записи и карта опроса ---------------------------------
    s7_write_burst_gap_sec: float = 1.0    # интервал короче — запись «внутри всплеска»
    s7_write_pause_sec: float = 20.0       # пауза длиннее — граница между всплесками
    s7_write_burst_min_jobs: int = 20      # минимум записей для оценки профиля
    pollmap_max_registers: int = 100_000   # максимум меток в карте опроса
